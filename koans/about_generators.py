...
class AboutGenerators(Koan):
    ...
    def test_generators_are_different_to_list_comprehensions(self):
        ...
        with self.assertRaises(TypeError): num = num_generator[0] # you cannot call an index on a generator without converting it to an iterable e.g list
    ...
    def test_generator_expressions_are_a_one_shot_deal(self):
        ...
        self.assertEqual(["Boom!", "Boom!", "Boom!"], attempt1) #the range starts at 0 and ends at 3 (exclusive)
    ...
    def test_generator_keeps_track_of_local_variables(self):
        ...
        self.assertEqual([2, 5, 9], list(result)) #ah I see, you can actually access the values in each iteration using yield.
    ...
    def test_generators_can_act_as_coroutines(self):
        generator = self.coroutine() # a coroutine is a function that can be suspended and resumed. Subroutine is a normal start-finish process.
