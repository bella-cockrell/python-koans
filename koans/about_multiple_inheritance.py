...
class AboutMultipleInheritance(Koan):
    ...
    def test_left_hand_side_inheritance_tends_to_be_higher_priority(self):
        jeff = self.Spiderpig()
        self.assertEqual("pink", jeff.color()) #pig was called first -- if Spider called first, it would be black

    def test_super_class_methods_are_higher_priority_than_super_super_classes(self):
        jeff = self.Spiderpig()
        self.assertEqual(8, jeff.legs()) #Sipder trumps Animal
    ...
    def test_confirm_the_mro_controls_the_calling_order(self):
        ...
        self.assertRegex(next.here(), "Spider") #next is next mro in this case
