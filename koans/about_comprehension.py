...
class AboutComprehension(Koan):
    ...
    def test_filtering_lists_with_list_comprehensions(self):
        ...
        comprehension = [delicacy for delicacy in feast if len(delicacy) > 6] #looks like this is another way to filter. add item if item in array is len(item) > 6
    ...
    def test_unpacking_tuples_in_list_comprehensions(self):
        ...
        comprehension = [ skit * number for number, skit in list_of_tuples ] #word * number for every number and word in the list
    ...
    def test_double_list_comprehension(self):
        ...
        comprehension = [ '{0} and {1}'.format(egg, meat) for egg in list_of_eggs for meat in list_of_meats] #iterates over items in eggs and meats are nested in every egg item
    ...
    def test_creating_a_dictionary_with_dictionary_comprehension(self):
        ...
        dict_comprehension = { k.upper(): weapon for k, weapon in dict_of_weapons.items() if weapon} #all of the keys have been uppered
        ...
        self.assertEqual(4, len(dict_comprehension)) #because you can't upper None

#and they say Python is readable. Worst unit so far.