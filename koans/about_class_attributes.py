...
class AboutClassAttributes(Koan):
    ...
    def test_objects_have_methods(self):
        fido = self.Dog()
        self.assertEqual(26, len(dir(fido))) #object is instantiated class
    ...
    def test_creating_objects_without_defining_a_class(self):
        singularity = object()
        self.assertEqual(23, len(dir(singularity))) #wonder what the difference is? __name__ could be one
    ...
    def test_defining_functions_on_individual_objects(self):
        fido = self.Dog()
        fido.wag = lambda : 'fidos wag' #lambda is anon/one line function
    ...
    class Dog2:
        def wag(self):
            return 'instance wag' #these are bound to the instance of the class
        ...
        @staticmethod
        def bark():
            return "staticmethod bark, arg: None" #static methods cannot access class attributes or instance attributes

        @classmethod
        def growl(cls):
            return "classmethod growl, arg: cls=" + cls.__name__ #this affects all instances of a class

    def test_since_classes_are_objects_you_can_define_singleton_methods_on_them_too(self):
        self.assertRegex(self.Dog2.growl(), "growl") #here, the instance method is being called

    def test_classmethods_are_not_independent_of_instance_methods(self):
        fido = self.Dog2()
        self.assertRegex(fido.growl(), "classmethod growl, arg: cls=Dog2") #class methods (available in all instances of the class) will overwrite instance methods of the same name
    ...
    def test_classmethods_can_not_be_used_as_properties(self):
        fido = self.Dog3()
        with self.assertRaises(TypeError): fido.name = "Fido" #this would work if fido._name = "Fido"
