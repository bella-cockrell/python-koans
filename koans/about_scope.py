...
class AboutScope(Koan):
    ...
    def test_you_can_reference_nested_classes_using_the_scope_operator(self):
        ...
        self.assertEqual(False, jims.Dog == joes.Dog) # though they use the same super, not the same instantiation
    ...
    def test_constants_are_assumed_by_convention_only(self):
        self.PI = "rhubarb"
        self.assertEqual("rhubarb", self.PI) #PI is overwritten
    ...
    def test_incrementing_with_local_counter(self):
        global counter #takes the value of 0
        start = counter #local var start is now 0 (diff mem address), global counter is 0
        self.increment_using_local_counter(start)
        self.assertEqual(False, counter == start + 1) #global counter is still 0, local var is now 1

    def test_incrementing_with_global_counter(self):
        global counter #is 0
        start = counter #new local var start is 0
        self.increment_using_global_counter() #global counter is now 1
